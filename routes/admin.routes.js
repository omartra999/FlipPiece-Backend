const express = require('express');
const adminRouter = express.Router();
const adminMiddleware = require('../middlewares/admin.middleware.js');
const orderController = require('../controllers/order.controller.js');
const productController = require('../controllers/product.controller.js');
const userController = require('../controllers/user.controller.js');
const { validateOrder } = require('../middlewares/validation.middleware.js');
const { validateProduct } = require('../middlewares/validation.middleware.js');
const { validateUserProfile } = require('../middlewares/validation.middleware.js');

adminRouter.get('/orders', adminMiddleware, validateOrder, orderController.getAllOrders);
adminRouter.get('/orders/:id', adminMiddleware, validateOrder, orderController.getOrderById);
adminRouter.put('/orders/:id/status', adminMiddleware, validateOrder, orderController.setOrderStatus);
adminRouter.get('/products', adminMiddleware, validateProduct, productController.getAllProducts);
adminRouter.get('/products/:id', adminMiddleware, validateProduct, productController.getProductById);
adminRouter.post('/products', adminMiddleware, validateProduct, productController.createProduct);
adminRouter.put('/products/:id', adminMiddleware, validateProduct, productController.updateProduct);
adminRouter.delete('/products/:id', adminMiddleware, validateProduct, productController.deleteProduct);
adminRouter.get('/users', adminMiddleware, validateUserProfile, userController.getAllUsers);
adminRouter.get('/users/:id', adminMiddleware, validateUserProfile, userController.getUserById);