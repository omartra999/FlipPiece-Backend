...
exports.getShippingQuote = async (req, res) => {
    try {
        const { destinationAddress, items, service } = req.body;
...
exports.markAsShipped = async (req, res) => {
    try {
        ...
        const order = await Order.findOne({ where: { dhlShipmentId: shipmentId } });
        
        if (!order) {
            return res.status(404).json({ error: 'Order not found for shipment' });
        }
