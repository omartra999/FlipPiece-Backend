...
describe('DHL Service', () => {
  ...
  describe('trackShipment', () => {
    ...
    it('should throw error on failure', async () => {
      ...
      await expect(dhlService.trackShipment('BADTRACK')).rejects.toEqual({ error: 'Not found' });
    });
  });

  describe('createShipment', () => {
    it('should return shipment data on success', async () => {
      axios.post.mockResolvedValue({ data: { shipmentId: 'SHIP123' } });
      const result = await dhlService.createShipment({ foo: 'bar' });
      expect(result).toEqual({ shipmentId: 'SHIP123' });
      expect(axios.post).toHaveBeenCalled();
    });

    it('should throw error on failure', async () => {
      axios.post.mockRejectedValue({ response: { data: { error: 'Invalid' } } });
      await expect(dhlService.createShipment({})).rejects.toEqual({ error: 'Invalid' });
    });
  });
}); 